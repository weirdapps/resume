...
- ESLint 10 for linting, `tsc --noEmit` for type-checking; no unit-test suite
...
- Do NOT bump TypeScript to 7. That major is blocked upstream and Dependabot is
  configured to skip it. TypeScript 7 breaks `next build` ("does not provide the
  compiler API required by Next.js") and makes `typescript-eslint` refuse to load,
  killing `npm run lint`. Full rationale in README.md under "Pinned major versions".
